...
| File | Description |
|------|-------------|
| `config.go` | `Config{Server, Temporal, Auth(OIDC), ArgoCD, OCI, GitHub, DocDB, OTel, Policies, Deploy}` with YAML tags. `Load(path)`: read → expand env → unmarshal → `applyDefaults` → `validate`. Secrets (`ArgoCD.Token`, `GitHub.Token`, `DocDB.ConnectionString`) carry `yaml:"-"` to keep them out of file serialization and must be set via env. `validate` requires: `temporal.hostPort`, `auth.oidc.audience`, `argocd.serverURL`, `oci.registry`, `otel.serviceName` |
...
- Defaults applied in `applyDefaults`: server `:8080` / 30s read / 60s write, temporal ns `default` / queue `deployment-workers`, OIDC issuer `https://token.actions.githubusercontent.com` / JWKS TTL 24h, OTel sample 1.0, deploy timeouts 600s/300s/20m/10s, DocDB collection `deployments`.
