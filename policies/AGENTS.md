...
- `tunableAllowlist` keys are **umbrella-relative** and every one is prefixed `app.` — the subchart ALIAS, never a chart name (the scaffolder rewrites names) — the deploy unit is the umbrella and the app is a subchart aliased within it. A root-scoped key validates a path the chart never reads. Never add a bare `app` entry: matching is prefix-based, so one blanket entry makes the whole subchart tunable.
